...
| API Gateway HTTP API | AWS managed | `infra/modules/api_gateway/` | Public entry point + auth gateway |
...
- **EKS Pod Identity.** All workloads (jenkins, cluster-autoscaler, fluent-bit, external-secrets, karpenter, ebs-csi-controller, efs-csi-controller) use Pod Identity associations — no node-level credentials shared.
