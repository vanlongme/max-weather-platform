...
By default the module ships a single repository:

| Key | Final repository name (with `var.name = "poc-max-weather"`) |
|-----|-------------------------------------------------------------|
| `api` | `poc-max-weather-api-repo` |

> Note: The `lambda-authorizer` ECR repository was removed from defaults — the Lambda authorizer is deployed as a ZIP package via the `lambda` module and does not use a container image. The Node.js base image is pulled directly from upstream `cgr.dev/chainguard/node` — no in-account mirror.
...
> Note: supplying `repositories` REPLACES the default map entirely. To keep the default repository alongside additional ones, the caller must re-declare it explicitly (e.g. include `api = {}`).
