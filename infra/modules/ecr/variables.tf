...
variable "repositories" {
  description = "Map of ECR repositories to create, keyed by short name. Each value is an object with optional per-repo overrides; when an override is null the module-level default applies. Final repository name is <var.name>-<key>-repo (e.g. key 'api' with var.name 'poc-max-weather' produces 'poc-max-weather-api-repo'). The default map creates the 'api' repository only. Note: the lambda-authorizer is deployed as a ZIP package via the lambda module — no ECR repo required. The Node.js base image is pulled directly from upstream cgr.dev/chainguard/node — no in-account ECR mirror."
  type = map(object({
