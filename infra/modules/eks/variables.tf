...
variable "eks_managed_node_groups" {
  description = "Map of EKS managed node group definitions, keyed by node group name. Each entry is passed through to the upstream eks_managed_node_groups input. Default AMI is Bottlerocket; default group is a single t3.medium 'infra' worker tainted role=infra:NoSchedule (cluster addon controllers tolerate it; Karpenter handles workload scale-out)."
  type = map(object({
