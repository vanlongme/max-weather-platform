...
- One or more managed node groups driven by the `eks_managed_node_groups` map
  (default: a single `infra` group — Bottlerocket x86_64, ON_DEMAND `t3.medium`,
  min 1 / max 10 / desired 1, labeled `role=infra` and tainted
  `role=infra:NoSchedule` — tagged for Cluster Autoscaler discovery; hosts
  cluster addon controllers (Deployments) and Karpenter while Karpenter scales
...
| Managed node group (`infra` key)      | `poc-max-weather-infra`                       |
...
| `eks_managed_node_groups` | Map of EKS managed node group definitions, keyed by group name. Passed through to the upstream `eks_managed_node_groups` input. | `map(object)` | single `infra` group (labeled `role=infra`, tainted `role=infra:NoSchedule`) | no |
