...
resource "aws_iam_role_policy" "pod_identity" {
  for_each = { for k, v in local.pod_identity_roles_effective : k => v if v.policy_json != null }
...
resource "aws_iam_role_policy_attachment" "pod_identity" {
  for_each = merge([
    for k, v in local.pod_identity_roles_effective : {
      for arn in coalesce(v.managed_policy_arns, []) : "${k}|${arn}" => {
        role = k
        arn  = replace(arn, var.partition_placeholder, data.aws_partition.current.partition)
      }
    }
  ...
  role       = aws_iam_role.pod_identity[each.value.role].name
  policy_arn = each.value.arn
}
