...
# __AWS_REGION__ / __AWS_ACCOUNT_ID__ / __CLUSTER_NAME__ placeholders that are
# substituted here. Defaults live in locals (not in the variable default) because
# Terraform forbids function calls — including jsonencode() — in variable defaults.
###############################################################################
...
resource "aws_iam_role_policy" "irsa" {
  ...
  policy = replace(
    replace(
      replace(each.value.policy_json, "__AWS_REGION__", var.aws_region),
