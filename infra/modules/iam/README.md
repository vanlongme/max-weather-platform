...
| `pod_identity_roles` | `pods.eks.amazonaws.com` (EKS Pod Identity) | All EKS workloads on 1.30+ | 6 built-in workload roles (jenkins, cluster-autoscaler, fluent-bit, external-secrets, ebs-csi-controller, efs-csi-controller) |
...
| `ebs-csi-controller`  | `poc-max-weather-ebs-csi-controller-role`       |
| `efs-csi-controller`  | `poc-max-weather-efs-csi-controller-role`       |
...
| `cluster_autoscaler_role_arn` / `fluent_bit_role_arn` / `external_secrets_role_arn` | Convenience accessors for three of the built-in Pod Identity roles. |
| `ebs_csi_controller_role_arn` / `efs_csi_controller_role_arn` | Convenience accessors for the EBS/EFS CSI driver Pod Identity role ARNs (fed into the `aws-ebs-csi-driver` / `aws-efs-csi-driver` EKS add-ons' `pod_identity_association`). |
