...
resource "helm_release" "fluent_bit" {
  ...
  version          = var.chart_versions["aws-for-fluent-bit"]
  namespace        = "amazon-cloudwatch"
  create_namespace = true
  values = [templatefile("${path.module}/values/aws-for-fluent-bit.yaml", {
    AWS_REGION     = var.aws_region
...
resource "kubernetes_manifest" "cluster_secret_store_aws" {
  manifest = yamldecode(templatefile("${path.module}/values/external-secrets-clustersecretstore.yaml", {
    AWS_REGION = var.aws_region
  }))
...
resource "kubernetes_manifest" "karpenter_ec2nodeclass_default" {
  manifest = yamldecode(templatefile("${path.module}/values/karpenter-ec2nodeclass.yaml", {
    CLUSTER_NAME                 = var.cluster_name
...
resource "kubernetes_manifest" "karpenter_nodepool_default" {
  manifest   = yamldecode(file("${path.module}/values/karpenter-nodepool.yaml"))
  depends_on = [kubernetes_manifest.karpenter_ec2nodeclass_default]
}
