...
.PHONY: help init plan apply destroy bootstrap \
...
api-gw-setup: ## Set up API Gateway HTTP API, VPC Link, and Lambda authorizer (use API_GW_ARGS=--dry-run for dry run)
	scripts/api-gw-setup.sh $(API_GW_ARGS)

deploy-staging: ## Deploy to staging via kubectl kustomize (addons are Terraform-managed)
	aws eks update-kubeconfig --name $(CLUSTER) --region $(REGION)
	kubectl apply -k k8s/overlays/staging
	kubectl rollout status deployment/weather-api -n $(NAMESPACE) --timeout=180s

deploy-prod: ## Deploy to prod via kubectl kustomize (addons are Terraform-managed)
	aws eks update-kubeconfig --name $(CLUSTER) --region $(REGION)
